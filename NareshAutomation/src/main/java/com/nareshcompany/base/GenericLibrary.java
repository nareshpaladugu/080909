package com.nareshcompany.base;
...
public class GenericLibrary extends Base {	
	 ...
	 * @throws Exception
	 */
	public void invokeWebApplication(String URL) throws TestFailedException, Exception {
		boolean bFlag = false;
		try {
			driver.get(URL);
	...
	public void selectCheckboxBasedOnIndex(int chkBoxindex) throws TestFailedException, Exception {
		...
		try {
			...
			oElement.get(chkBoxindex-1).click();
			bFlag=true;
		} catch (Exception ex) {
			bFlag = false;
	 ...
	 * @throws Exception
	 */
	public void selectValueFromDropDownByText(By locator, String visibleText) throws TestFailedException, Exception {
		boolean bFlag = false;
		try {
			waitForElementVisible(locator, 20);
			Select oDropDown = new Select(driver.findElement(locator));
			oDropDown.selectByVisibleText(visibleText);	    
			bFlag = true;
		} catch (Exception ex) {
			bFlag = false;
			...
			if (!bFlag)
				reports.failureReport("Select the value from the drop down", "Failed to select value from dropdown :" + visibleText);
		}
	 ...
	 * @throws Exception
	 */
	public void selectValueFromDropDownByIndex(By locator, int index) throws TestFailedException, Exception {
		boolean bFlag = false;
		try {
			Select oDropDown = new Select(driver.findElement(locator));
	...
	public void closeWindow() throws TestFailedException, Exception 
	{
		try {
			driver.close();
		} catch (Exception e) {
			reports.failureReport("Close window", "Not able close window");
		}
	}
	 ...
	 */
	public void selectFrame(String framename) throws TestFailedException, Exception 
	{
		try {
			driver.switchTo().frame(framename);
		} catch (Exception e) {
			reports.failureReport("Select Frame", "Not able select frame by name");
		}
	}
	 ...
	 */
	public void selectFrame(WebElement element) throws TestFailedException, Exception 
	{
		try {
			driver.switchTo().frame(element);
		} catch (Exception e) {
			reports.failureReport("Select Frame", "Not able select frame by element");
		}
	}
	 ...
	 */
	public void switchToDefaultContent(WebElement element) throws TestFailedException, Exception 
	{
		try {
			driver.switchTo().defaultContent();
		} catch (Exception e) {
			reports.failureReport("Select Page", "Not able switch to defaultContent");
		}
	}
	 ...
	 */
	public void selectWindow(int index) throws TestFailedException, Exception 
	{
		boolean bFlag = false;
		try {
			...
			bFlag = false;
			throw new TestFailedException("Failed to selected window " + ex.getMessage());
		}

		finally {
			if (!bFlag)
				reports.failureReport("Enter data based on locator", "Failed to selected window" + index);
		}
	 ...
	 * Verify innerText
	 */
	public void verifyinnerTextContains(By locator, String expectedMessage) throws TestFailedException, Exception {
		boolean bFlag = false;
		try {
			WebElement oElement = driver.findElement(locator);

			System.out.println("oElement.getAttribute(\"innerText\")... "+oElement.getAttribute("innerText"));
			if (!oElement.getAttribute("innerText").toLowerCase().contains(expectedMessage.toLowerCase())) {
				bFlag = false;
				throw new TestFailedException("Actual and Expected Messages should be same " + expectedMessage);
			}
			bFlag = true;
	 ...
	 * @throws TestFailedException
	 * @throws Exception
	 * Verify if given column is present in table or not
	 */
	public void verifyTableColumnPresent(WebElement table, String columnName) throws TestFailedException, Exception {

		boolean bFlag = false;
		try 
		{

			WebElement firstRow = table.findElement(By.tagName("tbody")).findElement(By.tagName("tr"));

			List<WebElement> allHeaders = firstRow.findElements(By.tagName("th"));

			for (WebElement singleCell : allHeaders) {

				if(singleCell.getText().equals(columnName))
				{
					bFlag = true;
					return;
				}
			...
			throw new TestFailedException("Table column not found " + columnName);
		}

		if (!bFlag)
			reports.failureReport("Verify Column name ", "Expected Table Coulmn Not Found");
	}
